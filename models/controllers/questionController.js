...
const getAllQuestions = async (req, res, next) => {
  try {
    ...
    const safe = escapeRegExp(queryText);
    const skip = (Number(page) - 1) * Number(limit);

    // Build filter: when no q, return recent questions; when q present, match title, description, or author fullname
    let filter = {};
    if (queryText) {
      const matchingUsers = await User.find({ fullname: { $regex: safe, $options: 'i' } }).select('_id');
      const authorIds = matchingUsers.map(u => u._id);
      const orConditions = [
        { title: { $regex: safe, $options: 'i' } },
        { description: { $regex: safe, $options: 'i' } },
      ];
      if (authorIds.length) orConditions.push({ author: { $in: authorIds } });
...
// Toggle like on a question (auth required)
const toggleLikeQuestion = async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.userid || (req.user && req.user._id);
    ...
    const isObjectId = String(id).match(/^[0-9a-fA-F]{24}$/);
    const question = await Question.findOne(isObjectId ? { _id: id } : { slug: id }).select('_id likedBy likesCount');
    if (!question) return next(new ErrorHandeler('Question not found', 404));

    const alreadyLiked = (question.likedBy || []).some((u) => String(u) === String(userId));
    const update = alreadyLiked
      ? { $pull: { likedBy: userId }, $inc: { likesCount: -1 } }
      : { $addToSet: { likedBy: userId }, $inc: { likesCount: 1 } };

    const updated = await Question.findByIdAndUpdate(question._id, update, { new: true }).select('likesCount likedBy');
...
// Record a share on a question (auth required)
const shareQuestion = async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.userid || (req.user && req.user._id);
    if (!userId) return next(new ErrorHandeler('Unauthorized', 401));
    ...
    const isObjectId = String(id).match(/^[0-9a-fA-F]{24}$/);
    const filter = isObjectId ? { _id: id } : { slug: id };
    const updated = await Question.findOneAndUpdate(filter, { $inc: { shareCount: 1 } }, { new: true }).select('shareCount');
    if (!updated) return next(new ErrorHandeler('Question not found', 404));
...
module.exports = { createQuestion, getAllQuestions, getQuestionById, updateQuestion, deleteQuestion, getQuestionsByAuthor, toggleLikeQuestion, shareQuestion };
